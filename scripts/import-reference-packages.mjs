/** Add the two reviewed PDF packages to the CMS; never replace an existing record.
 * Preview: node scripts/import-reference-packages.mjs
 * Save:    node scripts/import-reference-packages.mjs --write
 */
...
const files = [
  'coorg-2-nights-3-days-holiday-package.json',
  'skandagiri-sunrise-trek-from-bangalore.json',
];
...
const packages = files.map((file) => JSON.parse(fs.readFileSync(path.join('content/package-imports', file), 'utf8')));
...
const app = initializeApp({ credential: cert(account) }, 'reference-package-import');
...
try {
  ...
  if (!process.argv.includes('--write')) {
    ...
    pending.forEach((pkg, index) => batch.create(db.collection('packages').doc(pkg.id), {
      ...
      source: { kind: 'user-supplied-pdf', file: pkg.id.startsWith('coorg') ? 'Coorg 2 Nights - 3 Days Holiday Package.pdf' : 'Skandagiri Sunrise Trek.pdf' },
    }));
    ...
    console.log('Both packages already exist; no changes made.');
  }
} finally {
