...
export default function PackageDetailClient({ initialPackage }: { initialPackage: TravelPackage }) {
  ...
  const showInclusions = !hiddenSections.has("inclusions") && details.inclusions.length > 0;
  const showExclusions = !hiddenSections.has("exclusions") && details.exclusions.length > 0;
      ...
      <div className="mx-auto max-w-[1440px] px-4 py-7 sm:px-6 lg:px-8 lg:py-10">
        ...
        <div className="cmt-package-columns mt-7 grid items-start gap-7 lg:grid-cols-[minmax(0,1fr)_370px]">
          <div className="space-y-6">
            {!hiddenSections.has("about") && (details.summary.trim() || details.places.length > 0) ? <Section title="About this trip"><p className="text-sm leading-7 text-cmt-neutral-600">{details.summary}</p><div className="mt-5 flex flex-wrap gap-2">{details.places.map((place) => <span key={place} className="rounded-full border border-cmt-neutral-200 bg-cmt-neutral-50 px-3 py-1.5 text-xs font-semibold"><MapPin className="mr-1 inline size-3.5 text-cmt-primary-700" />{place}</span>)}</div></Section> : null}
            {!hiddenSections.has("highlights") && details.highlights.length > 0 && <Section title="Trip highlights"><div className="grid gap-3 sm:grid-cols-2">{details.highlights.map((item) => <div key={item} className="flex gap-3 rounded-cmt-control bg-cmt-primary-50 p-4 text-sm font-medium"><span className="grid size-6 shrink-0 place-items-center rounded-full bg-cmt-primary-500"><Check className="size-3.5" /></span>{item}</div>)}</div></Section>}
            {!hiddenSections.has("itinerary") && details.itinerary.length > 0 && <Section title="Day-by-day itinerary" eyebrow={`${details.itinerary.length} DAY PLAN`}><div className="space-y-3">{details.itinerary.map((day) => <details key={day.day} open={day.day === 1} className="group rounded-cmt-md border border-cmt-neutral-200 bg-white"><summary className="flex cursor-pointer list-none items-start gap-4 p-4 sm:p-5"><span className="grid size-10 shrink-0 place-items-center rounded-cmt-control bg-cmt-primary-500 text-xs font-bold">D{day.day}</span><span className="min-w-0"><b className="block font-display text-base">{day.title}</b><span className="mt-1 block text-xs text-cmt-neutral-500">{day.route || details.places[Math.min(day.day - 1, details.places.length - 1)]}</span></span><span className="ml-auto text-xl text-cmt-neutral-400 group-open:rotate-45">+</span></summary><div className="border-t border-cmt-neutral-100 px-4 py-4 text-sm leading-7 text-cmt-neutral-600 sm:px-[76px]"><p>{day.description}</p><p className="mt-2 text-xs font-semibold text-cmt-neutral-900">Meals: {day.meals || "Not specified"}</p></div></details>)}</div></Section>}
            {!hiddenSections.has("stays") && details.stays.length > 0 && <Section title="Comfort stays"><div className="grid gap-4 sm:grid-cols-2">{details.stays.map((stay, index) => <article key={`${stay.name}-${index}`} className="rounded-cmt-md border border-cmt-neutral-200 p-5"><BedDouble className="size-6 text-cmt-primary-700" /><h3 className="mt-4 font-display text-lg font-semibold">{stay.name}</h3><p className="mt-1 text-sm text-cmt-neutral-500">{stay.place} · {stay.nights} night{stay.nights === 1 ? "" : "s"}</p><p className="mt-3 text-sm leading-6 text-cmt-neutral-600">{stay.comfort}</p></article>)}</div></Section>}
            {(showInclusions || showExclusions) && <div className={`grid gap-6 ${showInclusions && showExclusions ? "sm:grid-cols-2" : ""}`}>{showInclusions && <ListSection title="Included" items={details.inclusions} positive />}{showExclusions && <ListSection title="Not included" items={details.exclusions} />}</div>}
            {!hiddenSections.has("cancellation") && details.cancellationPolicy.trim() && <Section title="Cancellation policy"><div className="flex gap-3 rounded-cmt-control bg-cmt-success-100 p-4"><ShieldCheck className="size-5 shrink-0 text-cmt-success-700" /><p className="text-sm leading-6 text-cmt-neutral-700">{details.cancellationPolicy}</p></div></Section>}
            <PackagePageSections value={pageSections} />
          </div>
