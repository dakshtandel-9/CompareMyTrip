...
export default function QuoteModal({ pkg, initialTravellers, initialTravelDate = "", onClose }: { pkg: TravelPackage; initialTravellers: number; initialTravelDate?: string; onClose: () => void }) {
  const authUser = useAuthUser();
  const profileState = useUserProfile();

  return <Modal onClose={onClose} labelledBy="quote-modal-title" className="cmt-quote-modal fixed inset-0 z-[100] grid overflow-y-auto bg-cmt-secondary-900/70 px-4 py-6 backdrop-blur-sm"><button tabIndex={-1} type="button" aria-label="Close quote form" onClick={onClose} className="fixed inset-0 cursor-default" /><div className="relative m-auto w-full max-w-xl overflow-hidden rounded-cmt-lg bg-white shadow-cmt-xl">
    {profileState.status === "loading" || authUser === undefined ? <div className="grid min-h-80 place-items-center"><div role="status"><h2 id="quote-modal-title" className="sr-only">Loading quote form</h2><LoaderCircle aria-hidden="true" className="size-8 animate-spin text-cmt-primary-700" /></div></div> : profileState.status === "ready" && authUser ? <QuoteForm key={`${profileState.profile.email}-${profileState.profile.name}-${profileState.profile.phone}-${initialTravellers}-${initialTravelDate}`} pkg={pkg} profile={profileState.profile} userId={authUser.uid} initialTravellers={initialTravellers} initialTravelDate={initialTravelDate} onClose={onClose} /> : <div className="p-8 text-center"><h2 id="quote-modal-title" className="font-semibold">Please sign in to request a customized quote.</h2><button type="button" onClick={onClose} className="mt-4 min-h-11 px-4">Close</button></div>}
  </div></Modal>;
}

function QuoteForm({ pkg, profile, userId, initialTravellers, initialTravelDate, onClose }: { pkg: TravelPackage; profile: UserProfile; userId: string; initialTravellers: number; initialTravelDate: string; onClose: () => void }) {
  const [name, setName] = useState(profile.name);
  const [phone, setPhone] = useState(profile.phone || "+91");
  // Prefilled from the booking card, so the date is not asked for twice.
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    // The picker cannot offer a day the trip does not run, but the state it
    // writes to is older than the rule — a package edited mid-session could
    // leave a stale date behind.
    if (!isDepartureAllowed(pkg, travelDate)) {
      return setError(`This trip departs ${runsOnLabel.toLowerCase()}. Pick another date.`);
    }
  ...
  return <><header className="flex items-start gap-3 border-b border-cmt-neutral-200 px-5 py-4 sm:px-6"><span className="grid size-11 shrink-0 place-items-center rounded-cmt-control bg-cmt-primary-500 font-display text-sm font-bold">{pkg.destination.slice(0, 2).toUpperCase()}</span><div className="min-w-0 flex-1"><h2 id="quote-modal-title" className="font-display text-xl font-semibold">Get customized quotes</h2><p className="mt-0.5 truncate text-xs text-cmt-neutral-500">{pkg.title}</p></div><button type="button" onClick={onClose} data-modal-initial-focus aria-label="Close" className="grid size-9 place-items-center rounded-cmt-control text-cmt-neutral-400 hover:bg-cmt-neutral-100 hover:text-cmt-neutral-900"><X className="size-5" /></button></header>
    <form onSubmit={submit} className="max-h-[calc(100vh-9rem)] overflow-y-auto p-5 sm:p-6" noValidate>
      ...
      <div className="space-y-4"><label className="block"><span className="mb-1.5 block text-sm font-semibold">Full name *</span><span className="relative block"><UserRound className="absolute left-4 top-1/2 size-4 -translate-y-1/2 text-cmt-neutral-400" /><input required autoComplete="name" value={name} onChange={(event) => setName(event.target.value)} className={`${inputClass} pl-11`} /></span></label>
        ...
        <div className="grid gap-4 sm:grid-cols-[minmax(0,3fr)_minmax(190px,2fr)]"><PhoneNumberField required compactCountryCode value={phone} onChange={setPhone} /><div className="block min-w-0"><span className="mb-2 flex items-baseline justify-between gap-2 text-sm font-semibold text-cmt-neutral-700">Travel date *{runsOnLabel ? <span className="shrink-0 rounded-cmt-full bg-cmt-primary-100 px-2 py-0.5 text-xs font-semibold text-cmt-neutral-800">{runsOnLabel}</span> : null}</span><DepartureDatePicker value={travelDate} onChange={setTravelDate} allowedDays={runsOn} triggerClassName="h-12" /></div></div>
        <label className="block"><span className="mb-1.5 block text-sm font-semibold">How many travellers? *</span><span className="relative block"><Users className="absolute left-4 top-1/2 size-4 -translate-y-1/2 text-cmt-neutral-400" /><input required type="number" min="1" max="20" step="1" value={travellers} onChange={(event) => setTravellers(event.target.value)} className={`${inputClass} pl-11`} /></span></label>
