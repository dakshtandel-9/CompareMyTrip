...
export default function BookingCard({ bookingUnavailableReason, pkg, details, travelDate, onTravelDateChange, travellers, onTravellersChange, onRequestQuote }: Props) {
    ...
    <div className="cmt-package-booking-card overflow-hidden rounded-[20px] border border-[#e5e5e0] bg-white text-cmt-neutral-900 shadow-[0_8px_32px_rgba(25,35,31,0.055)]">
      ...
      <div className="cmt-package-booking-body space-y-5 p-5 sm:p-6">
        ...
        <div className="cmt-package-booking-actions space-y-2.5">
          {bookingUnavailableReason ? <button type="button" disabled aria-describedby="booking-unavailable-note" className="flex min-h-[52px] w-full items-center justify-center rounded-[10px] bg-cmt-primary-500 px-3 py-3 text-[15px] font-bold text-cmt-neutral-900 shadow-[0_3px_10px_rgba(205,158,17,0.12)] transition-[background-color,box-shadow,transform] duration-200 hover:bg-cmt-primary-600 hover:shadow-[0_5px_14px_rgba(205,158,17,0.18)] motion-safe:hover:-translate-y-px focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-cmt-primary-500 disabled:cursor-not-allowed">Book Now</button> : (
<Link
            href={checkoutHref}
          ...
          {bookingUnavailableReason && <p id="booking-unavailable-note" className="px-1 text-center text-xs leading-relaxed text-cmt-neutral-500">{bookingUnavailableReason}</p>}
          ...
          <button
            type="button"
            disabled={Boolean(bookingUnavailableReason)}
