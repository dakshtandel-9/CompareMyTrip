  ...
  return <div className="space-y-6">
    <header className="flex flex-wrap items-start justify-between gap-4">
      <div className="max-w-2xl">
        <p className="text-xs font-semibold tracking-wide text-cmt-primary-900">{eyebrow}</p>
    ...
    <dl className="grid gap-3 sm:grid-cols-3">
      {metrics.map(metric => <div key={metric.label} className={`rounded-2xl border p-5 ${metric.attention ? "border-cmt-primary-100 bg-cmt-primary-50/60" : "border-slate-200 bg-white"}`}>
...
export function WorkflowGuide({ steps }: { steps: string[] }) {
  return <div className="my-5 flex flex-wrap items-center gap-x-3 gap-y-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-xs leading-5 text-slate-600" aria-label="How to use this page">
    ...
    {steps.map((step, index) => <span className="inline-flex items-center gap-3" key={step}>
      ...
      <span><span className="mr-1.5 font-semibold text-cmt-primary-900">{index + 1}.</span>{step}</span>
    </span>)}
  </div>;
}
...
export function InboxSearch({ value, onChange, placeholder = "Search name, email, phone or destination", label = "Search records" }: {
  ...
  return <label className="relative block w-full sm:max-w-sm">
    ...
    <input type="search" value={value} onChange={event => onChange(event.target.value)} placeholder={placeholder}
      className="h-11 w-full rounded-xl border border-slate-200 bg-white pl-9 pr-3 text-sm text-slate-900 outline-none placeholder:text-slate-400 focus:border-cmt-primary-500 focus:ring-2 focus:ring-cmt-primary-100" />
  </label>;
}
...
export function InboxFilters<Value extends string>({ value, onChange, options }: {
  ...
  return <div className="flex flex-wrap gap-2" aria-label="Filter records">
    {options.map(option => <button key={option.value} type="button" aria-pressed={value === option.value} onClick={() => onChange(option.value)}
      className={`inline-flex min-h-10 items-center gap-2 rounded-lg px-3 text-sm font-medium transition-colors ${value === option.value ? "bg-cmt-primary-50 text-cmt-primary-900 ring-1 ring-inset ring-cmt-primary-100" : "text-slate-500 hover:bg-slate-50 hover:text-slate-900"}`}>
...
export function InboxState({ loading, empty, title, description, onReset }: {
  ...
  if (loading) return <div role="status" className="flex items-center justify-center gap-3 px-5 py-16 text-sm text-slate-500"><LoaderCircle className="size-5 animate-spin text-cmt-primary-800" aria-hidden="true" />Loading records…</div>;
...
export function ContactLinks({ email, phone }: { email: string; phone: string }) {
  return <div className="mt-2 flex flex-wrap gap-x-4 gap-y-2 text-sm text-slate-600">
    {phone && <a href={`tel:${phone}`} className="inline-flex items-center gap-1.5 hover:text-cmt-primary-900"><Phone className="size-3.5 shrink-0" aria-hidden="true" />{phone}</a>}
    {email && <a href={`mailto:${email}`} className="inline-flex min-w-0 items-center gap-1.5 break-all hover:text-cmt-primary-900"><Mail className="size-3.5 shrink-0" aria-hidden="true" />{email}</a>}
