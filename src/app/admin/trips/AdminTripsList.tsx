      ...
      <div className="divide-y divide-slate-100">
        {visible.map(trip => <article key={trip.id} className="p-5 sm:p-6">
          ...
          <div className="mt-4 grid gap-4 rounded-xl bg-slate-50 p-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <label><span className="mb-1.5 block text-xs font-medium text-slate-500">Departure date</span><input type="date" value={trip.tripDate} disabled={Boolean(updatingId) || trip.paymentStatus !== "successful"} onChange={event => void changeDate(trip, event.target.value)} aria-label={`Departure date for ${trip.txnid}`} className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none focus:border-cmt-primary-500 focus:ring-2 focus:ring-cmt-primary-100 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-400" />
            ...
            <label><span className="mb-1.5 block text-xs font-medium text-slate-500">Trip status</span><select value={trip.tripStatus} disabled={Boolean(updatingId) || trip.paymentStatus !== "successful"} onChange={event => void changeStatus(trip, event.target.value as TripStatus)} aria-label={`Trip status for ${trip.txnid}`} className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none focus:border-cmt-primary-500 focus:ring-2 focus:ring-cmt-primary-100 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-400">{(Object.keys(TRIP_STATUS_LABELS) as TripStatus[]).map(status => <option key={status} value={status}>{TRIP_STATUS_LABELS[status]}</option>)}</select><span className="mt-1 block text-xs text-slate-500">{trip.paymentStatus === "successful" ? "Changes save automatically" : "Available after payment is confirmed"}</span></label>
          </div>
          <details className="group mt-4"><summary className="flex w-fit cursor-pointer list-none items-center gap-2 rounded-md py-1 text-sm font-semibold text-cmt-primary-900 [&::-webkit-details-marker]:hidden"><ChevronDown className="size-4 transition-transform group-open:rotate-180" aria-hidden="true" />View payment & booking details</summary><dl className="mt-4 grid gap-4 rounded-xl border border-slate-200 p-4 text-sm sm:grid-cols-2 lg:grid-cols-3">
            ...
            {trip.discount > 0 && <div><dt className="text-xs text-slate-500">Discount applied</dt><dd className="mt-1 text-cmt-primary-900">{trip.couponCode} · {formatINR(trip.discount)} off {formatINR(trip.subtotal)}</dd></div>}
          </dl></details>
        </article>)}
