...
export default function PackageDetailEditor({ form, pkg, section, onSectionChange, onChange, change, onUploadImages, disabled, filedUnderOptions }: {
  ...
  filedUnderOptions: Record<TravelPackage["region"], string[]>;
}) {
  ...
  const text = (label: string, key: "summary" | "places" | "highlights" | "transfers" | "inclusions" | "exclusions" | "cancellationPolicy", hint?: string) => <PackageContentField label={label} value={form[key]} onChange={value => set(key, value)} hint={hint} disabled={disabled} />;
  ...
  return <div className={styles.workspace}>
    <nav className={styles.sectionNav} aria-label="Package editor sections">
    ...
    <section className={styles.formPanel} aria-labelledby="package-editor-heading">
      <header><p className={styles.eyebrow}>{current.group}</p><h2 id="package-editor-heading" tabIndex={-1}>{current.label}</h2><p>Changes stay in this draft until you save. Preview the page at any time.</p></header>
      <fieldset disabled={disabled} className={styles.fields} aria-busy={disabled}>
        ...
        {section === "basics" && <>
          ...
          <div className={styles.fieldGrid}>
            ...
            <label><FieldLabel>Filed under destination</FieldLabel><input className={inputClass} value={form.destination} list="package-destinations" onChange={event => set("destination", event.target.value)} /><datalist id="package-destinations">{filedUnderOptions[form.region].map(value => <option key={value} value={value} />)}</datalist></label>
        ...
        {section === "overview" && <>
          <PackageContentField label="Tagline (optional)" value={form.pageSections.tagline ?? ""} onChange={value => note("tagline", value)} />
          <PackageContentField label="Introduction (optional)" value={form.pageSections.introduction ?? ""} onChange={value => note("introduction", value)} />
        ...
        {section === "itinerary" && <>
          ...
          {form.itinerary.map((day, index) => <details key={`day-${index}`} open={index === 0} className={styles.item}>
            ...
            <div className={styles.itemFields}>
              ...
              <PackageContentField label={`Day ${day.day} description`} value={day.description} onChange={value => change(["details", "itinerary", index, "description"], value)} />
              ...
              {(day.activities ?? []).map((activity, activityIndex) => <div key={activityIndex} className={styles.itemFields}>
                ...
                <PackageContentField label={`Day ${day.day}, activity ${activityIndex + 1}: description`} value={activity.description} onChange={value => change(["details", "itinerary", index, "activities", activityIndex, "description"], value)} />
          ...
          <PackageContentField label="Itinerary note (optional)" value={form.pageSections.itineraryNote ?? ""} onChange={value => note("itineraryNote", value)} />
        </>}
        {section === "stays" && <>
          {visibility("stays")}
          {form.stays.map((stay, index) => <details key={index} open={index === 0} className={styles.item}><summary>{stay.name || `Stay ${index + 1}`}</summary><div className={styles.itemFields}>
            ...
            <PackageContentField label={`Stay ${index + 1}: description`} value={stay.comfort} onChange={value => change(["details", "stays", index, "comfort"], value)} />
          ...
          <PackageContentField label="Stay note (optional)" value={form.pageSections.stayNote ?? ""} onChange={value => note("stayNote", value)} />
        </>}
        ...
        {section === "coverage" && <>{visibility("inclusions")}{text("Included", "inclusions", "One included service per line.")}{visibility("exclusions")}{text("Not included", "exclusions", "One excluded service per line.")}<PackageContentField label="Inclusions note (optional)" value={form.pageSections.inclusionNote ?? ""} onChange={value => note("inclusionNote", value)} /></>}
