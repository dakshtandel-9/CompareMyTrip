...
export default function AdminPackageBuilder({ initialPackage, filedUnderOptions, onCancel, onSaved }: { initialPackage?: TravelPackage; filedUnderOptions: Record<TravelPackage["region"], string[]>; onCancel: () => void; onSaved: () => void }) {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <section className="rounded-cmt-md border border-cmt-neutral-200 bg-white p-5 shadow-cmt-sm sm:p-7">
              ...
              <div className="mt-6 grid gap-5 sm:grid-cols-2">
                ...
                <label><FieldLabel>Filed under</FieldLabel><input list="filed-under-options" value={form.destination} onChange={(e) => update("destination", e.target.value)} placeholder={form.region === "India" ? "Select a state" : "Select or type a country"} autoComplete="off" className={inputClass} /><datalist id="filed-under-options">{filedUnderOptions[form.region].map((destination) => <option key={destination} value={destination} />)}</datalist><span className="mt-1.5 block text-xs text-cmt-neutral-500">{form.region === "India" ? "Pick the state this package is in. It appears in the site\u2019s destination filter as soon as this package is saved." : "Choose an existing destination or type a new one. New destinations are created when this package is saved."}</span></label>
