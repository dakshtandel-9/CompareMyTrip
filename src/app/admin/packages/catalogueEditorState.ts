...
export function packageValidationIssue(form: PackageForm): EditorIssue | null {
  ...
  if (!hidden.includes("about") && (!form.summary.trim() || !form.places.split(/[,·]/).some((place) => place.trim()))) return issue("Add a package overview and at least one place.", 3);
  if (!hidden.includes("itinerary") && (!form.itinerary.length || form.itinerary.some((day) => !day.title.trim()))) return issue("Add a title for every itinerary day.", 4);
  if (!hidden.includes("stays") && (!form.stays.length || form.stays.some((stay) => !stay.name.trim()))) return issue("Add a name for every hotel or stay.", 4);
  ...
  const pageIssue = form.pageSections ? packagePageSectionsIssue(form.pageSections) : null;
  if (pageIssue) return issue(pageIssue, 6);
  ...
  if (!hidden.includes("stays") && form.stays.some((stay) => !Number.isInteger(stay.nights) || stay.nights < 1)) return issue("Enter at least 1 whole night for each stay.", 4);
  return null;
}
