        ...
        <form noValidate onSubmit={handleSubmit} className="space-y-6">
          ...
          <section hidden={step !== 3} className="rounded-cmt-md border border-cmt-neutral-200 bg-white p-5 shadow-cmt-sm sm:p-7">
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              {([{ value: "draft", label: "Keep as a draft", description: "Save privately until your article is ready." }, { value: "published", label: "Publish on the website", description: "Make this article available to readers when you save." }] as const).map((option) => <label key={option.value} className={`flex cursor-pointer items-start gap-3 rounded-xl border p-4 ${post.status === option.value ? "border-cmt-primary-400 bg-cmt-primary-50" : "border-cmt-neutral-200"}`}><input type="radio" name="article-status" checked={post.status === option.value} onChange={() => update("status", option.value)} className="mt-1 size-4 accent-cmt-primary-700" /><span><span className="block text-sm font-semibold">{option.label}</span><span className="mt-1 block text-xs leading-5 text-cmt-neutral-500">{option.description}</span></span></label>)}
            </div>
            <div className="mt-4">
              <button
                type="button"
          ...
          <div className="sticky bottom-3 z-10 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-cmt-neutral-200 bg-white/95 p-4 shadow-cmt-md backdrop-blur-sm">
            ...
            {step < 3 ? <button disabled={saving || Boolean(uploadingKey)} type="button" onClick={() => goToStep(step + 1)} className="inline-flex h-11 items-center gap-2 rounded-xl bg-cmt-primary-500 px-5 text-sm font-semibold text-cmt-neutral-900 disabled:opacity-50">Continue <ArrowRight className="size-4" /></button> : <button disabled={saving || Boolean(uploadingKey)} type="submit" className="inline-flex h-11 items-center gap-2 rounded-xl bg-cmt-primary-500 px-5 text-sm font-semibold text-cmt-neutral-900 disabled:opacity-50"><Save className="size-4" />{saving ? "Saving…" : post.status === "draft" ? "Save as draft" : "Save and publish"}</button>}
          </div>
        </form>
