...
const SUPPORT_PHONES = [
  ...
  { label: "Email us", number: "support@comparemytrip.in", href: "mailto:support@comparemytrip.in" },
];
    ...
    <ul className={footerRow ? "grid gap-4 lg:grid-cols-2 lg:gap-5 xl:grid-cols-4" : compact ? "mt-6 space-y-4" : "mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4"}>
      {SUPPORT_PHONES.map((phone) => (
        <li key={phone.href}>
          ...
          >
            {phone.href.startsWith("mailto:")
              ? <Mail className="mt-1 h-4 w-4 shrink-0 text-cmt-primary-700" aria-hidden="true" />
